...
import io.gatling.core.Predef.{atOnceUsers, clock, configuration, csv, exec, openInjectionProfileFactory, scenario}
...
class FeederJmapAllScenario extends Simulation {
  ...
  val users = csv("users.csv").circular.readRecords
    .map({ record =>    User(Username(record("username").toString), Password(record("password").toString)) })
